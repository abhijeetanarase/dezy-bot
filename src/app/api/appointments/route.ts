...
import { handleAvailableDoctors, handleBookAppointment, handleCancelAllAppointments, handleCancelAppointment, handleCancelAppointmentById, handleCheckSlotAvailability, handleGetAllAvailableSlots, handleGetDoctorAppointments, handleGetDoctorsInfo, handleGetPatientAppointments, handleInfoAboutDoctor, handleSelectDoctor } from "@/app/utils/opreations";
...
export async function POST(req: Request) {
  ...
  console.log("Middleware payload:", { userId, userEmail });
    ...
    const systemPrompt = `You are an AI assistant for Dezy Clinic, a plastic surgery clinic.
  ...
  Response Example : 
{
  ...
  console.log("System Prompt:", systemPrompt);
    ...
    if (cleanedResult?.type === "function_call") {
        ...
        try {
            switch (cleanedResult.function_call.name) {
                ...
                default:
                    return NextResponse.json({ error: "Unknown function call" }, { status: 400 });
            }
        } catch (error) {
    ...
    return NextResponse.json({ message: cleanedResult?.message || "No action taken" });
}
