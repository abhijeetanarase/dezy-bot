...
export default function ChatPage() {
  ...
  const [cancelState , setCancelState] = useState<{ id: string | null; text: string } | null>({id : null, text: ''});
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const formattedInput = input  + `${selectedDoctor ? `with  doctorID : ${selectedDoctorId}` : ''} ${slectedSlot ? `and slot : ${slectedSlot}` : '' + `${cancelState?.id ? ` and cancel appointment : ${cancelState.id}` : ''}`}`;
      ...
      if (data.data) {
        if (Array.isArray(data.data)) {
          ...
          setMessages((prev) => [
            ...
            {
              id: Date.now().toString(),
              text: data.message || 'Your appointment has been booked successfully!',
              sender: 'bot',
            },
          ]);
          setShowForm(false);
  ...
  const bookSlot = (slot: any) => {
    setInput(` I want to book on  date : ${new Date(slot.start).toLocaleDateString()}, start: ${new Date(slot.start).toLocaleTimeString()}, end: ${new Date(slot.end).toLocaleTimeString()}`);
      ...
      <header className="bg-teal-600 text-white p-4 shadow-md">
        <div className="container mx-auto flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-xl font-bold">Dezy Clinic Booking Assistant</h1>
              <p className="text-teal-100 text-sm">Your personal healthcare scheduling companion</p>
            </div>
          </div>
          <button 
            onClick={() => {
              ...
              >
                ...
                {message.doctors && (
                  <div className="mt-3 space-y-3">
                    <p className="font-medium text-teal-700">Available Doctors:</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {message.doctors.map((doctor: any) => (
                        <div
                          key={doctor._id}
                          className="border border-teal-100 p-3 rounded-lg cursor-pointer hover:bg-teal-50 transition-colors"
                          onClick={() => handleDoctorSelect(doctor)}
                        >
                          <p className="font-medium text-teal-800">Dr. {doctor.name}</p>
                ...
                {message.slots && (
                  <div className="mt-3 space-y-3">
                    <p className="font-medium text-teal-700">
                      Available Slots on {new Date(message.slots[0].start).toLocaleDateString()}
                    </p>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {message.slots.map((slot: any, index) => (
                        <div
                          key={index}
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-500">Click to book</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {message.appointment && message.appointment.length > 0 && (
                  <div className="mt-3 space-y-3">
                    <p className="font-medium text-teal-700">Your Appointments:</p>
                    <AppointmentList appointments={message.appointment} message={message.text} setCancelState={setCancelState}/>
                  </div>
                )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-teal-800">Book Appointment</h3>
                <button 
